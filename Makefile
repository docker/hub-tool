...
.PHONY: build
build: ## Build docker-hub in a container
...
.PHONY: cross
cross: ## Cross compile docker-hub binaries in a container
	docker build $(BUILD_ARGS) . \
	--output type=local,dest=./dist \
	--target cross

.PHONY: install
install: build ## Install docker-hub to your local cli-plugins directory
...
.PHONY: e2e-build
e2e-build:
	docker build $(BUILD_ARGS) . --target e2e -t docker-hub:e2e

.PHONY: e2e
e2e: e2e-build ## Run the end-to-end tests
	@docker run $(E2E_ENV) --rm -v /var/run/docker.sock:/var/run/docker.sock -v $(shell go env GOCACHE):/root/.cache/go-build docker-hub:e2e

test-unit-build:
	docker build $(BUILD_ARGS) . --target test-unit -t docker-hub:test-unit

.PHONY: test-unit
test-unit: test-unit-build ## Run unit tests
	docker run --rm -v $(shell go env GOCACHE):/root/.cache/go-build docker-hub:test-unit
